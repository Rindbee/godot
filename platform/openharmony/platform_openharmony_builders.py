...
def generate_bundle(target, source, env):
    ...
    app_dir = os.path.join(project_path, "bin", f"openharmony_{env['target']}")

    # if os.path.exists(app_dir):
    #     shutil.rmtree(app_dir)
    if not os.path.exists(app_dir):
        template_dir = os.path.join(project_path, "misc", "dist", "openharmony_template")
    ...
    target_dir = os.path.join(app_dir, "entry", "libs", lib_dest_arch)
